...
@section('content')
                    ...
                    <form action="/pinjaman">
                        <div class="row">
                            <div class="col-lg-6 col-sm-12">
                                <x-inputtext label="Dari" field="dari" value="{{ Request('dari') }}"
                                    icon="feather icon-calendar" datepicker />
                            </div>
                            <div class="col-lg-6 col-sm-12">
                                <x-inputtext label="Sampai" field="sampai" value="{{ Request('sampai') }}"
                                    icon="feather icon-calendar" datepicker />
                            </div>
                        </div>
                        @php
                        ...
                        @if (Auth::user()->kode_cabang == 'PCF' && in_array($level, $level_search))
                            <div class="row">
                                ...
                                <div class="col-lg-3 col-sm-12">
                                    <div class="form-group">
                                        <x-inputtext label="Nama Karyawan" value="{{ Request('nama_karyawan') }}"
                                ...
                                <div class="col-lg-2 col-sm-12">
                                    <div class="form-group">
                                        <select name="status" id="status" class="form-control">
                                            ...
                                            <option value="0" {{ Request('status') == 0 ? 'selected' : '' }}>Belum di
                                                Proses</option>
                                            <option value="1" {{ Request('status') == 1 ? 'selected' : '' }}>Sudah di
                                                Proses</option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <tbody>
                                @foreach ($pinjaman as $d)
                                    <tr>
                                        ...
                                        <td>{!! $d->jumlah_pinjaman - $d->totalpembayaran == 0
                                            ? '<span class="badge bg-success">L</span>'
                                            : '<span class="badge bg-danger">BL</span>' !!}</td>
                                        <td>
                                        ...
                                        @endif --}}
                                                ...
                                                <a class="ml-1 show" no_pinjaman="{{ $d->no_pinjaman }}"
                                                    href="#"><i class="feather icon-file-text info"></i></a>
                                                <a href="/pinjaman/{{ Crypt::encrypt($d->no_pinjaman) }}/cetakformulir"
                                                    target="_blank" class="ml-1"><i
                                                        class="feather icon-printer text-primary"></i></a>


                                                @if ($d->status == null && Auth::user()->id == $d->id_user)
                                                    <a class="ml-1 edit" no_pinjaman="{{ $d->no_pinjaman }}"
                                                        href="#"><i class="feather icon-edit success"></i></a>
                                                    <form method="POST" class="deleteform"
                                                        action="/pinjaman/{{ Crypt::encrypt($d->no_pinjaman) }}/delete">
                                                        @csrf
                                                        @method('DELETE')
    ...
    <div class="modal fade text-left" id="mdlprosespinjaman" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel18" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <div class="modal fade text-left" id="mdlshowpinjaman" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel18" aria-hidden="true">
    ...
    <div class="modal fade text-left" style="z-index: 1052 !important" id="mdlinputbayarpinjaman" tabindex="-1"
