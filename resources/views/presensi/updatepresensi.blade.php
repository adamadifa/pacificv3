            ...
            @if (empty($kode_jadwal))
                ...
                @if ($level != 'manager hrd' && $level != 'admin' && $level != 'spv presensi')
                    <select name="status" id="status" class="form-control">
                        ...
                        <option value="a" {{ $cek != null ? ($cek->status == 'a' ? 'selected' : '') : '' }}>Alfa
                        </option>
                    </select>
                    <div class="row statushadir mt-2">
                        <div class="col-12">
                            <select name="kode_jadwal" id="kode_jadwal" class="form-control">
                                ...
                                @foreach ($jadwal as $d)
                                    <option
                                        {{ $cek != null ? ($cek->kode_jadwal == $d->kode_jadwal ? 'selected' : '') : '' }}
                                        value="{{ $d->kode_jadwal }}">{{ $d->nama_jadwal }} - {{ $d->kode_cabang }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                @else
                    <div class="row">
                        <div class="col-12">
                            <select name="status" id="status" class="form-control">
                                <option value="">Status Kehadiran</option>
                                <option value="h"
                                    {{ $cek != null ? ($cek->status == 'h' ? 'selected' : '') : '' }}>
                                    Hadir</option>
                                <option value="i"
                                    {{ $cek != null ? ($cek->status == 'i' ? 'selected' : '') : '' }}>
                                    Izin</option>
                                <option value="s"
                                    {{ $cek != null ? ($cek->status == 's' ? 'selected' : '') : '' }}>
                                    Sakit</option>
                                <option value="a"
                    ...
                    <div class="row mt-1" id="statusjam">
                        <div class="col-6">
                            <x-inputtext label="Jam Masuk"
                                value="{{ $cek != null ? ($cek->jam_in != null ? date('H:i', strtotime($cek->jam_in)) : '') : '' }}"
                                field="jam_masuk" icon="feather icon-clock" />

                        </div>
                        <div class="col-6">
                            <x-inputtext label="Jam Pulang"
                                value="{{ $cek != null ? ($cek->jam_out != null ? date('H:i', strtotime($cek->jam_out)) : '') : '' }}"
                                field="jam_pulang" icon="feather icon-clock" />
                        </div>
                    </div>
                @endif
            @else
                ...
                @else
                    ...
                    <div class="row mt-1 statushadir">
                        <div class="col-6">
                            <x-inputtext label="Jam Masuk" readonly
                                value="{{ $cek != null ? ($cek->jam_in != null ? date('H:i', strtotime($cek->jam_in)) : '') : '' }}"
                                field="jam_masuk" icon="feather icon-clock" />
                        ...
                        <div class="col-6">
                            <x-inputtext label="Jam Pulang" readonly
                                value="{{ $cek != null ? ($cek->jam_out != null ? date('H:i', strtotime($cek->jam_out)) : '') : '' }}"
                                field="jam_pulang" icon="feather icon-clock" />
                        </div>
