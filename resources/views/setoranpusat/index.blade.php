...
@section('content')
                ...
                <div class="card">
                    <div class="card-header">
                        @if (in_array($level, $setoranpusat_add))
                            <a href="#" class="btn btn-primary" id="inputsetoranpusat"><i class="fa fa-plus mr-1"></i>
                                Tambah Data</a>
                        @endif
                    </div>
                    <div class="card-body">
                                        ...
                                        <select name="kode_bank" id="kode_bank" class="form-control">
                                            ...
                                            @foreach ($bank as $d)
                                                <option {{ Request('kode_bank') == $d->kode_bank ? 'selected' : '' }}
                                                    value="{{ $d->kode_bank }}">{{ $d->nama_bank }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <a href="/setoranpusat/cetak?dari={{ Request('dari') }}&sampai={{ Request('sampai') }}&kode_cabang={{ Request('kode_cabang') }}&kode_bank={{ Request('kode_bank') }}&excel=false"
                            target="_blank" class="btn btn-primary"><i class="feather icon-printer"></i></a>
                        <a href="/setoranpusat/cetak?dari={{ Request('dari') }}&sampai={{ Request('sampai') }}&kode_cabang={{ Request('kode_cabang') }}&kode_bank={{ Request('kode_bank') }}&excel=true"
                                ...
                                <tbody>
                                    @foreach ($setoranpusat as $d)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $d->nama_bank }}</td>
                                            <td class="text-right">
                                                {{ !empty($d->uang_kertas) ? rupiah($d->uang_kertas) : '' }}</td>
                                            <td class="text-right">
                                                {{ !empty($d->uang_logam) ? rupiah($d->uang_logam) : '' }}</td>
                                            <td class="text-right">{{ !empty($d->transfer) ? rupiah($d->transfer) : '' }}
                                            </td>
                                            <td class="text-right">{{ !empty($d->giro) ? rupiah($d->giro) : '' }}</td>
                                            <td class="text-right">{{ !empty($totalsetoran) ? rupiah($totalsetoran) : '' }}
                                            </td>
                                            <td>
                                                @if ($d->status == 1)
                                                    <span class="badge bg-success"><i class="fa fa-check"></i> Diterima
                                                        {{ date('d-m-Y', strtotime($d->tgl_diterimapusat)) }}</span>
                                                @elseif($d->status == 2)
                                                    <span class="badge bg-danger"><i class="fa fa-close"></i> Ditolak</span>
                                                @else
                                                    <span class="badge bg-warning"><i class="fa fa-history"></i> Belum
                                                        Diterima</span>
                                                @endif
                                            </td>
                                            <td>
                                                        ...
                                                        @if (in_array($level, $setoranpusat_edit))
                                                            @if (empty($d->no_ref))
                                                                <a class="ml-1 edit" status="{{ $d->status }}"
                                                                    kode_setoranpusat="{{ $d->kode_setoranpusat }}"
                                                                    href="#"><i
                                                        ...
                                                        @if (in_array($level, $setoranpusat_hapus))
                                                            @if ($d->status == 0)
                                                                <form method="POST" class="deleteform"
                                                                    action="/setoranpusat/{{ Crypt::encrypt($d->kode_setoranpusat) }}/delete">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <a href="#" tanggal="{{ $d->tgl_setoranpusat }}"
    ...
    <div class="modal fade text-left" id="mdlinputsetoranpusat" tabindex="-1" role="dialog"
    ...
    <!-- Edit Setoran Pusat -->
    <div class="modal fade text-left" id="mdleditsetoranpusat" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel18" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <!-- Penerimaan Setoran -->
    <div class="modal fade text-left" id="mdlterimasetoran" tabindex="-1" role="dialog"
        aria-labelledby="myModalLabel18" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
    ...
    <script>
        $(function() {
            ...
            $(".terimasetoran").click(function(e) {
                ...
                $("#loadterimasetoeran").load("/setoranpusat/" + kode_setoranpusat +
                    "/createterimasetoran");
            });
