...
<form method="POST" action="/kasbon/store" id="frmKasbon">
        ...
        <div class="col-12">
            <table class="table table-bordered">
                <tr>
                    <th>NIK</th>
                    <td>
                        <input type="hidden" name="nik" id="nik" value="{{ $kasbon->nik }}">
                        {{ $kasbon->nik }}
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th>Perusahaan</th>
                    <td>{{ $kasbon->id_perusahaan=="MP" ? "Makmur Permata" : "CV.Pacific Tasikmalaya" }}</td>
                </tr>
                <tr>
                    <th>Kantor</th>
                    <td>{{ $kasbon->nama_cabang }}</td>
                </tr>
                <tr>
                    <th>Masa Kerja</th>
                    <td>
                        @php
                        $awal = date_create($kasbon->tgl_masuk);
                ...
                <tr>
                    <th>Status</th>
                    <td>
                        <input type="hidden" name="status_karyawan" id="status_karyawan" value="{{ $kasbon->status_karyawan }}">
                        {{ $kasbon->status_karyawan=="T" ? "Karyawan Tetap" : "Karyawan Kontrak" }}
                    </td>
                </tr>
                @if ($kasbon->status_karyawan == "K")
                <tr>
                    <th>Akhir Kontrak</th>
                    <td>
                        <input type="hidden" name="id_jabatan" id="id_jabatan" value="{{ $kasbon->id_jabatan }}">
                        <input type="hidden" name="akhir_kontrak" id="akhir_kontrak" value="{{ $kasbon->akhir_kontrak }}">
                        {{ $kasbon != null ? DateToIndo2($kasbon->akhir_kontrak) : "" }}
                    </td>
                </tr>
                @endif
                <tr>
    ...
    <div class="row">
        ...
        <div class="col-8">
            <x-inputtext label="Tanggal Kasbon" value="{{ $kasbon->tgl_kasbon }}" field="tgl_kasbon" icon="feather icon-calendar" datepicker />
        </div>
    </div>
    <div class="row">
        <div class="col-4">
        ...
        <div class="col-8">
            <x-inputtext label="Jumlah Kasbon" value="{{ rupiah($kasbon->jumlah_kasbon) }}" field="jml_kasbon" icon="feather icon-file" right />
        </div>
    </div>
    <div class="row">
        <div class="col-4">
        ...
        <div class="col-8">
            <x-inputtext label="Jatuh Tempo" value="{{ $kasbon->jatuh_tempo }}" field="jatuh_tempo" icon="feather icon-calendar" readonly />
        </div>
    </div>
    <div class="row">
        <div class="col-12">
