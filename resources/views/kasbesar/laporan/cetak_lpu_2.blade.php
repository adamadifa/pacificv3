                ...
                @foreach ($salesman as $d)
                    @php
                        $setoran = DB::table('setoran_penjualan')
                            ->selectRaw(
                                'SUM(IFNULL(setoran_kertas,0) + IFNULL(setoran_logam,0) + IFNULL(setoran_transfer,0) + IFNULL(setoran_bg,0) + IFNULL(setoran_lainnya,0)) as jmlsetoran',
                            )
                            ->where('tgl_lhp', $dari)
                            ->where('id_karyawan', $d->id_karyawan)
                            ->groupByRaw('tgl_lhp,id_karyawan')
