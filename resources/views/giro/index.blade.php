...
@section('content')
                                ...
                                <div class="col-lg-3 col-sm-12">
                                    <div class="form-group">
                                        <select name="status" id="status" class="form-control">
                                            ...
                                            <option {{ Request('status') == '0' ? 'selected' : '' }} value="0">Pending
                                            </option>
                                            <option {{ Request('status') == '1' ? 'selected' : '' }} value="1">Diterima
                                            </option>
                                            <option {{ Request('status') == '2' ? 'selected' : '' }} value="2">Ditolak
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    @foreach ($giro as $d)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @elseif($d->status == 1)
                                                    <span
                                                        class="badge bg-success">{{ date('d-m-Y', strtotime($d->tglbayar)) }}</span>
                                                @elseif($d->status == 2)
                                                    ...
                                                    @if (in_array($level, $giro_hapus))
                                                        @if ($d->status == 0)
                                                            <form method="POST" class="deleteform"
                                                                action="/pembayaran/{{ Crypt::encrypt($d->no_giro) }}/deleteallnogiro">
                                                                @csrf
                                                                @method('DELETE')
    ...
    <!-- Detail Giro -->
    <div class="modal fade text-left" id="mdldetailfaktur" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18"
        aria-hidden="true">
    ...
    <!-- Proses Giro -->
    <div class="modal fade text-left" id="mdlprosesgiro" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
