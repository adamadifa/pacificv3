...
@section('content')
                ...
                <div class="card">
                    @if (in_array($level, $salesman_tambah))
                        <div class="card-header">
                            <a href="/salesman/create" class="btn btn-primary"><i class="fa fa-plus mr-1"></i> Tambah
                                Data</a>
                        </div>
                    @endif
                    <div class="card-body">
                        <form action="/salesman">
                            <div class="row">
                                <div class="col-lg-4 col-sm-12">
                                    <x-inputtext label="Nama Salesman" field="nama" icon="feather icon-user"
                                        value="{{ Request('nama') }}" />
                                </div>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group" aria-label="Basic example">
                                                    @if (in_array($level, $salesman_edit))
                                                        <a class="ml-1"
                                                            href="/salesman/{{ \Crypt::encrypt($d->id_karyawan) }}/edit"><i
                                                                class="feather icon-edit success"></i></a>
                                                    @endif
                                                    <a class="ml-1 detailsalesman" href="#"
                                                        idkaryawan="{{ Crypt::encrypt($d->id_karyawan) }}"><i
                                                            class=" feather icon-file-text info"></i></a>
                                                    @if (in_array($level, $salesman_hapus))
                                                        <form method="POST" class="deleteform"
                                                            action="/salesman/{{ Crypt::encrypt($d->id_karyawan) }}/delete">
                                                            @csrf
                                                            @method('DELETE')
    ...
    <!-- Detail Salesman -->
    <div class="modal fade text-left" id="mdldetailsalesman" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18"
        aria-hidden="true">
