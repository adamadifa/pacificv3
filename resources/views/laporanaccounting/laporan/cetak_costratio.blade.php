    ...
    <table class="datatable3" @if (empty($kode_cabang)) style="width:100%" @endif border="1">
            ...
            @foreach ($biaya as $key => $d)
                ...
                <tr>
                    ...
                    @foreach ($cbg as $b)
                        ...
                        @endphp
                        <td style="text-align:right">{{ !empty($d->$field_cabang) ? rupiah($d->$field_cabang) : '' }}
                        </td>
                    @endforeach
                    @if (empty($kode_cabang))
        ...
        <tfoot>
            ...
            <tr>
                ...
                @foreach ($cbg as $f)
                    @php
                        $kode_cbg = strtolower($f->kode_cabang);
                    @endphp
                    <th style="background-color:rgb(210, 59, 4); color:white"> {{ ${"cr_aida_piutang_$kode_cbg"} }}%
                    </th>
                @endforeach
                @if (empty($kode_cabang))
            ...
            <tr>
                ...
                @if (empty($kode_cabang))
                    <th style="background-color:rgba(145, 2, 59, 0.961); color:white">
                        {{ $cr_aida_biayapiutang_total }}%
                    </th>
                @endif
            </tr>
            <tr>
                <th style="background-color:rgba(145, 2, 59, 0.961); color:white" colspan="3">COST RATIO SWAN + AIDA
                </th>
                @foreach ($cbg as $f)
                    @php
                        $kode_cbg = strtolower($f->kode_cabang);
