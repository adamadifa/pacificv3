                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-header">
                            <a href="#" id="tambahgaji" class="btn btn-primary"><i class="fa fa-plus mr-1"></i> Tambah Data</a>
                        </div>
                        @endif
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover-animation">
                                    ...
                                    <tbody>
                                        @foreach ($gaji as $d)
                                        <tr>
                                            <td class="text-center">{{ $loop->iteration + $gaji->firstItem()-1 }}</td>
                                            <td>{{ $d->kode_gaji }}</td>
                                            <td>{{ $d->nik }}</td>
                                            ...
                                            <td class="text-right">{{ date("d-m-Y",strtotime($d->tgl_berlaku)) }}</td>
                                            <td>
                                                <div class="btn-group" role="group" aria-label="Basic example">
                                                    @if (in_array($level,$gaji_edit))
                                                    <a class="ml-1 edit" kode_gaji="{{ Crypt::encrypt($d->kode_gaji) }}" href="#"><i class="feather icon-edit success"></i></a>
                                                    @endif
                                                    @if (in_array($level,$gaji_hapus))
                                                    <form method="POST" class="deleteform" action="/gaji/{{Crypt::encrypt($d->kode_gaji)}}/delete">
                                                        @csrf
                                                        @method('DELETE')
...
<!-- Input Gaji -->
<div class="modal fade text-left" id="mdlinputgaji" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18" aria-hidden="true">
...
<div class="modal fade text-left" id="mdleditgaji" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
