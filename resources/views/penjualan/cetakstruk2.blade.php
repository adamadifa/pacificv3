<div id="print" style="position: absolute; z-index:1;  background-color:white">
            ...
            <div>
                ...
                @if (!empty($jumlah_dus))
                    <div style="display: flex; justify-content:space-between">
                        <span>{{ $jumlah_dus }} Dus x
                            {{ rupiah($d->harga_dus) }}</span><span>{{ rupiah($jumlah_dus * $d->harga_dus) }}</span>
                    </div>
                @endif
                @if (!empty($jumlah_pack))
                    <div style="display: flex; justify-content:space-between">
                        <span>{{ $jumlah_pack }} Pack x
                            {{ rupiah($d->harga_pack) }}</span><span>{{ rupiah($jumlah_pack * $d->harga_pack) }}</span>
                    </div>
                @endif
                @if (!empty($jumlah_pcs))
                    <div style="display: flex; justify-content:space-between">
                        <span>{{ $jumlah_pcs }} Pcs x
                            {{ rupiah($d->harga_pcs) }}</span><span>{{ rupiah($jumlah_pcs * $d->harga_pcs) }}</span>
                    </div>
                @endif
            </div>
        @endforeach
    ...
    <div class="row">
        <div class="col-12">
            <button class="btn btn-info btn-block" id="printstruk"
                onclick="BtPrint(document.getElementById('pre_print').innerText)"><i
                    class="feather icon-printer mr-1"></i>Cetak Faktur
            </button>
        </div>
    </div>
</div>
...
function maxLen($input)
{
    ...
    foreach ($input as $entry) {
        if (strlen($entry->kode_produk) > $length) {
            $length = strlen($entry->kode_produk);
        }
    }
    return $length;
}
