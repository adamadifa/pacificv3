        ...
        <tbody>
            @foreach ($barang as $b)
                @php
                    ...
                    if ($b->kode_produk == 'P1000') {
                        $isipcsdusP1000 = $b->isipcsdus;
                    }
                @endphp
            @endforeach
            ...
            @foreach ($penjualan as $d)
                @php
                    ...
                    if (!empty($d->retur_BR20)) {
                        $returBR20 = $d->retur_BR20 / $isipcsdusBR20;
                    } else {
                        $returBR20 = 0;
                    }
                    ...
                    //P1000
                    if (!empty($d->P1000)) {
                        $P1000 = $d->P1000 / $isipcsdusP1000;
                    } else {
                        $P1000 = 0;
                    }
                    if (!empty($d->retur_P1000)) {
                        $returP1000 = $d->retur_P1000 / $isipcsdusP1000;
                    } else {
                        $returP1000 = 0;
                    }
                    ...
                    $totalreturBR20 += $returBR20;
                    $totalreturP1000 += $returP1000;
                ...
                <tr style="font-size: 12px; background-color: {{ $color }}">
                    ...
                    <td align="right"><b>@php
                        if (!empty($d->totalretur)) {
                            echo rupiah($d->totalretur);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->potaida)) {
                            echo rupiah($d->potaida);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->potswan)) {
                            echo rupiah($d->potswan);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->potstick)) {
                            echo rupiah($d->potstick);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->potsp)) {
                    ...
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->potistimewa)) {
                            echo rupiah($d->potistimewa);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->ppn)) {
                    ...
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->totalnetto)) {
                            echo rupiah($d->totalnetto);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->jenistransaksi)) {
                            echo ucwords($d->jenistransaksi);
                        }
                    @endphp</b></td>
                    <td align="right"><b>@php
                        if (!empty($d->totalbayar)) {
                            echo rupiah($d->totalbayar);
                        }
                    @endphp</b></td>
                    <td align="center"><b>@php
                        if (!empty($d->lastpayment)) {
                            echo date('d-m-y', strtotime($d->lastpayment));
                        }
                    @endphp</b></td>
                    <td align="center" style="color: {{ $colortext }};"><b>{{ $ket }}
                            {{ $d->date_updated }}</b></td>
                </tr>
                <tr>
            ...
            <tr style="background-color: #024a75; color:white">
                ...
                <th align="right"><b>{{ desimal($totalP1000) }}</b></th>
    ...
    {{-- <table class="datatable3" style="width:40%" border="1">
        ...
        <tbody>
            <tr>
                ...
                <th align="right"><b>{{ desimal($BBDP) }} - {{ desimal($returBBDP) }} =
                        {{ desimal($BBDP - $returBBDP) }} </b></th>
                <th align="right"><b>{{ desimal($SP8) }} - {{ desimal($returSP8) }} =
                        {{ desimal($SP8 - $returSP8) }} </b></th>
                <th align="right"><b>{{ desimal($SP) }} - {{ desimal($returSP) }} = {{ desimal($SP - $returSP) }}
                    </b></th>
                <th align="right"><b>{{ desimal($AR) }} - {{ desimal($returAR) }} = {{ desimal($AR - $returAR) }}
                    </b></th>
                <th align="right"><b>{{ desimal($ABAS) }} - {{ desimal($returABAS) }} =
                        {{ desimal($ABAS - $returABAS) }} </b></th>
            </tr>
        </tbody>
    </table> --}}
