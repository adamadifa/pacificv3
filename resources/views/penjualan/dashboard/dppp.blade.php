            ...
            @foreach ($dppp as $d)
                @php
                    ...
                    if (!empty($realisasi_bulanini_tahunlalu)) {
                        $grw_bulanini = (($realisasi_bulanini_tahunini - $realisasi_bulanini_tahunlalu) / $realisasi_bulanini_tahunlalu) * 100;
                    } else {
                        $grw_bulanini = 0;
                    }

                    if (!empty($realisasi_sampaibulanini_tahunlalu)) {
                        $grw_sampaibulanini = (($realisasi_sampaibulanini_tahunini - $realisasi_sampaibulanini_tahunlalu) / $realisasi_sampaibulanini_tahunlalu) * 100;
                    } else {
                ...
                <tr>
                    ...
                    <td align="right" class="{{ $ach_bulanini < 100 ? 'text-danger' : 'text-info' }} font-weight-bold">
                        {{ desimal($ach_bulanini) }}</td>
                    ...
                    <td align="right"
                        class="{{ $ach_sampaibulanini < 100 ? 'text-danger' : 'text-info' }}  font-weight-bold">
                        {{ desimal($ach_sampaibulanini) }}</td>
