...
@section('content')
                    ...
                    <form action="/penjualan" id="frmPenjualan">
                        ...
                        <div class="row">
                                    ...
                                    <div class="form-group">
                                        <select name="id_karyawan" id="id_karyawan" class="form-control select2">
                                            ...
                                            @foreach ($salesman as $d)
                                                <option {{ Request('id_karyawan') == $d->id_karyawan ? 'selected' : '' }}
                                                    value="{{ $d->id_karyawan }}">{{ $d->nama_karyawan }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-lg-2 col-sm-10">
                                <div class="form-group">
                                    <select name="status" id="status" class="form-control">
                                        ...
                                        <option {{ Request('status') == '2' ? 'selected' : '' }} value="2">PRE ORDER
                                        </option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-1 col-sm-2">
                                <button type="submit" name="submit" value="1" class="btn btn-primary search"><i
                        ...
                        @if (Auth::user()->level != 'salesman')
                            <div class="row mb-1">
                                <div class="col-12">
                                    <button class="btn btn-info" type="submit" name="print" id="cetaksuratjalan"
                                        value="submit"><i class="feather icon-printer mr-1"></i> Cetak Surat Jalan</button>
                                </div>
                            </div>
                        @endif
                                        ...
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            @if (in_array($level, $penjualan_edit))
                                                @if (substr($d->no_fak_penj, 3, 2) == 'PR')
                                                    <a
                                                        href="/penjualan/{{ Crypt::encrypt($d->no_fak_penj) }}/updatenofaktur">
                                                        <i class="feather icon-sliders warning"></i>
                                                    </a>
                                                @endif

                                                <a class="ml-1"
                                                    href="/penjualan/{{ \Crypt::encrypt($d->no_fak_penj) }}/editv2"><i
                                                        class="feather icon-edit success"></i></a>


                                                @if (Auth::user()->level != 'salesman')
                                                    <a class="ml-1 detailpenjualan"
                                                        href="/penjualan/{{ Crypt::encrypt($d->no_fak_penj) }}/show"><i
                                                            class=" feather icon-file-text info"></i></a>
                                                @else
                                                    <a class="ml-1 detailpenjualan"
                                                        href="/penjualan/{{ Crypt::encrypt($d->no_fak_penj) }}/showforsales"><i
                                                            class=" feather icon-file-text info"></i></a>
                                                @endif
                                            @endif
                                            @if (in_array($level, $penjualan_hapus))
                                                <form method="POST" name="deleteform" class="deleteform"
                                                    action="/penjualan/{{ Crypt::encrypt($d->no_fak_penj) }}/delete">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="#" tanggal="{{ $d->tgltransaksi }}"
                                            ...
                                            @if (Auth::user()->level != 'salesman')
                                                @if ($d->status == 1)
                                                    <a href="/penjualan/{{ Crypt::encrypt($d->no_fak_penj) }}/updatepending"
                                                        class="ml-1"><i class="fa fa-refresh accent-1"></i></a>
                                                @endif
                                                <div class="dropdown ml-1">
                                                    <a class="dropdown-toggle mr-1" type="button"
                                                        id="dropdownMenuButton300" data-toggle="dropdown"
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton300"
                                                        ...
                                                        <a class="dropdown-item" target="_blank"
                                                            href="/penjualan/cetakfaktur/{{ Crypt::encrypt($d->no_fak_penj) }}"><i
                                                                class="feather icon-printer mr-1"></i>Cetak Faktur</a>
                                                        <a class="dropdown-item" target="_blank"
                                                            href="/penjualan/cetaksuratjalan/{{ Crypt::encrypt($d->no_fak_penj) }}/1"><i
                                                                class="feather icon-printer mr-1"></i>Cetak Surat Jalan
                                                            1</a>
                                                        <a class="dropdown-item" target="_blank"
                                                            href="/penjualan/cetaksuratjalan/{{ Crypt::encrypt($d->no_fak_penj) }}/2"><i
                                                                class="feather icon-printer mr-1"></i>Cetak Surat Jalan
                                                            2</a>
                                                    </div>
                                                </div>
                                                @if ($d->nama_pelanggan != 'BATAL')
                                                    <a href="#" class="danger ubahfakturbatal"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/penjualan/setfakturbatal" method="POST">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <textarea name="keterangan" placeholder="Keterangan" id="keteranan" cols="30" rows="10"
    ...
    <div class="modal fade text-left" id="mdlshowfaktur" tabindex="-1" role="dialog" aria-labelledby="myModalLabel18"
        aria-hidden="true">
