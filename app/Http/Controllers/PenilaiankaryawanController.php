...
class PenilaiankaryawanController extends Controller
{
    public function index($kategori_jabatan, $perusahaan, Request $request)
    {
        ...
        $query->select('hrd_penilaian.kode_penilaian', 'tanggal', 'hrd_penilaian.nik', 'nama_karyawan', 'hrd_penilaian.id_kantor', 'hrd_penilaian.periode_kontrak', 'hrd_penilaian.kode_dept', 'nama_dept', 'hrd_penilaian.id_jabatan', 'nama_jabatan', 'kp', 'ka', 'rsm', 'm', 'gm', 'hrd', 'dirut', 'status', 'pemutihan', 'no_kb', 'hrd_kontrak.no_kontrak', 'masa_kontrak_kerja');
    ...
    public function cetak($kode_penilaian)
    {
        ...
        $historipemutihan = DB::table('hrd_bayarjmk')->where('nik', $penilaian->nik)->orderBy('tgl_pembayaran')->get();
        if ($kategori == 1) {
            return view('penilaiankaryawan.cetak', compact('tanggal', 'dari', 'sampai', 'karyawan', 'kategori_penilaian', 'kategori', 'penilaian', 'approve', 'inisial', 'histori_kontrak', 'historipemutihan'));
        } else {
            return view('penilaiankaryawan.cetak_operator', compact('tanggal', 'dari', 'sampai', 'karyawan', 'kategori_penilaian', 'kategori', 'penilaian', 'approve', 'inisial', 'histori_kontrak', 'historipemutihan'));
        }
    }
