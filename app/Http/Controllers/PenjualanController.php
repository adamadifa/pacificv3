...
class PenjualanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($request->print)) {
            ...
            if ($this->cabang != "PCF" and $this->cabang != "PST") {
                ...
                $query->whereIn('cabangbarunew', $cabang);
            }
            $penjualan = $query->get();
            ...
            if (!empty($request->no_fak_penj)) {
                $query->where('penjualan.no_fak_penj', $request->no_fak_penj);
            }
            ...
            if (Auth::user()->level == "salesman") {
                $query->where('salesbarunew', Auth::user()->id_salesman);
            }
            // if ($this->cabang != "PCF") {
            //     if ($this->cabang == "GRT") {
            //         $query->where('karyawan.kode_cabang', 'TSM');
    ...
    public function cetaklaporanpenjualan(Request $request)
    {
        ...
        if (!empty($request->kode_cabang)) {
            if ($jenislaporan == "standar") {
                ...
                retur_AB,retur_AR,`retur_AS`,retur_BB,retur_CG,retur_CGG,retur_DEP,retur_DK,retur_DS,retur_SP,retur_BBP,retur_SPP,retur_CG5,retur_SC,retur_SP8,retur_SP500,retur_BR20,
                P1000,
                penjualan.subtotal as totalbruto,
                (ifnull( r.totalpf, 0 ) - ifnull( r.totalgb, 0 ) ) AS totalretur,
                penjualan.penyharga AS penyharga,
    ...
    public function cetakroutingsalesman(Request $request)
    {
        ...
        if ($jenislaporan == "detail") {
            ...
            return view('penjualan.laporan.cetak_routingsalesman', compact('cabang', 'salesman', 'dari', 'sampai', 'penjualan'));
        } else {
            $query = Salesman::query();
            $query->selectRaw('karyawan.id_karyawan,nama_karyawan,IFNULL(jmlkunjungan,0)  as totalkunjungan,IFNULL(jmlsesuaijadwal,0)  as totalsesuaijadwal');
            $query->leftJoin(
                DB::raw("(
                    SELECT
                    penjualan.id_karyawan,
    ...
    public function cetaklaporanharganet(Request $request)
    {
        ...
        $harganet = DB::table('penjualan')
            ...
            SUM(bruto_BR20) as bruto_BR20,
            SUM(bruto_BR20_tunai) as bruto_BR20_tunai,
            SUM(bruto_BR20_kredit) as bruto_BR20_kredit,

            SUM(bruto_P1000) as bruto_P1000,
            SUM(bruto_P1000_tunai) as bruto_P1000_tunai,
            SUM(bruto_P1000_kredit) as bruto_P1000_kredit,
            ...
            SUM(qtydus_BR20) as qtydus_BR20,
            SUM(qtydus_P1000) as qtydus_P1000,
            ...
            + IFNULL(qtydus_SC,0)
            + IFNULL(qtydus_SP8,0)
            + IFNULL(qtydus_SP500,0)) as qtySWAN,

            SUM(
                IFNULL(IFNULL(potaida,0) /
                (IFNULL(qtydus_AB,0)
                + IFNULL(qtydus_AR,0)
                + IFNULL(qtydus_AS,0)
                + IFNULL(qtydus_CG,0)
            ...
            SUM(
                IFNULL((IFNULL(potswan,0) + IFNULL(potstick,0) + IFNULL(potsp,0)) /
                (IFNULL(qtydus_BB,0)
                + IFNULL(qtydus_BBP,0)
                + IFNULL(qtydus_DEP,0)
                + IFNULL(qtydus_DS,0)
            ...
            * qtydus_AS,0)) as diskon_AS,
            ...
            SUM(IFNULL(retur_BR20,0)) as retur_BR20,
            SUM(IFNULL(retur_P1000,0)) as retur_P1000
        ")
            ->leftJoin(
                DB::raw("(
                SELECT
                ...
                SUM(IF(b.kode_produk = 'BR20',dp.subtotal,0)) as bruto_BR20,
                SUM(IF(b.kode_produk = 'BR20' AND jenistransaksi='tunai',dp.subtotal,0)) as bruto_BR20_tunai,
                SUM(IF(b.kode_produk = 'BR20' AND jenistransaksi='kredit',dp.subtotal,0)) as bruto_BR20_kredit,

                SUM(IF(b.kode_produk = 'P1000',dp.subtotal,0)) as bruto_P1000,
                SUM(IF(b.kode_produk = 'P1000' AND jenistransaksi='tunai',dp.subtotal,0)) as bruto_P1000_tunai,
                SUM(IF(b.kode_produk = 'P1000' AND jenistransaksi='kredit',dp.subtotal,0)) as bruto_P1000_kredit,
                ...
                SUM(IF(b.kode_produk = 'P1000' AND promo != 1 OR b.kode_produk = 'P1000' AND promo IS NULL,dp.jumlah,0)) as qty_P1000,
                ...
                SUM(IF(b.kode_produk = 'BR20' AND promo !=1 OR b.kode_produk = 'BR20' AND promo IS NULL,floor(dp.jumlah/mb.isipcsdus),0)) as   qtydus_BR20,
                SUM(IF(b.kode_produk = 'P1000' AND promo !=1 OR b.kode_produk = 'P1000' AND promo IS NULL,floor(dp.jumlah/mb.isipcsdus),0)) as   qtydus_P1000
                FROM detailpenjualan dp
                INNER JOIN barang b ON dp.kode_barang = b.kode_barang
            ...
            ->leftJoin(
                ...
                SUM(IF(b.kode_produk = 'BR20',subtotal,0)) as   retur_BR20,
                SUM(IF(b.kode_produk = 'P1000',subtotal,0)) as   retur_P1000
                FROM detailretur
		...
		SUM(IF(barang.kode_produk = 'BR20',subtotal,0)) as   retur_BR20,
		SUM(IF(barang.kode_produk = 'P1000',subtotal,0)) as   retur_P1000,
		...
		SUM(IF(barang.kode_produk = 'BR20' AND jenis_retur='GB',subtotal,0)) as   returpeny_BR20,
		SUM(IF(barang.kode_produk = 'P1000' AND jenis_retur='GB',subtotal,0)) as   returpeny_P1000
        ")
            ->join('retur', 'detailretur.no_retur_penj', '=', 'retur.no_retur_penj')
            ->join('barang', 'detailretur.kode_barang', '=', 'barang.kode_barang')
            ->join('master_barang', 'barang.kode_produk', '=', 'master_barang.kode_produk')
        ...
        if ($jenislaporan == 1) {
            ...
            $query->leftJoin(
                DB::raw("(
                    ...
                    SUM( IF ( kode_produk = 'P1000', detailpenjualan.subtotal, NULL ) ) AS P1000
                    FROM
                        detailpenjualan
                        INNER JOIN barang ON detailpenjualan.kode_barang = barang.kode_barang
    ...
    public function inputbarang(Request $request)
    {
        ...
        return view('penjualan.inputbarang', compact('barang', 'pengurangharga'));
    }
    ...
    public function cetakrekaptandatangan(Request $request)
    {
        ...
        if (empty($kode_cabang)) {
            ...
            $query->selectRaw(
                ...
                SUM(IF(penjualan.signature IS NULL,1,0)) belumada'
            );
            $query->join('karyawan', 'penjualan.id_karyawan', '=', 'karyawan.id_karyawan');
            $query->join('pelanggan', 'penjualan.kode_pelanggan', '=', 'pelanggan.kode_pelanggan');
            $query->orderBy('karyawan.kode_cabang');
