...
DB::raw("(
SELECT kode_cabang,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='AB',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='AB',jumlah,0)),0) as mutasi_ab,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='AR',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='AR',jumlah,0)),0) as mutasi_ar,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='AS',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='AS',jumlah,0)),0) as mutasi_as,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='BB',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='BB',jumlah,0)),0) as mutasi_bb,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='DEP',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='DEP',jumlah,0)),0) as mutasi_dep,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='DS',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='DS',jumlah,0)),0) as mutasi_ds,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='SP',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='SP',jumlah,0)),0) as mutasi_sp,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='SPP',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='SPP',jumlah,0)),0) as mutasi_spp,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='SC',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='SC',jumlah,0)),0) as mutasi_sc,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='SP8',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='SP8',jumlah,0)),0) as mutasi_sp8,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='SP8-P',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='SP8-P',jumlah,0)),0) as mutasi_sp8p,
IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='SP500',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='SP500',jumlah,0)),0) as mutasi_sp500,

IFNULL(SUM(IF(inout_good ='IN' AND kode_produk ='BR20',jumlah,0)),0) - IFNULL(SUM(IF(inout_good ='OUT' AND kode_produk
='BR20',jumlah,0)),0) as mutasi_br20
...
AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='SURAT JALAN' OR tgl_mutasi_gudang_cabang>= (SELECT
    MAX(saldomax.tanggal)
    FROM saldoawal_bj saldomax
    WHERE saldomax.kode_cabang = mc.kode_cabang)
    AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='TRANSIT IN' OR tgl_mutasi_gudang_cabang>= (SELECT
        MAX(saldomax.tanggal)
        FROM saldoawal_bj saldomax
        WHERE saldomax.kode_cabang = mc.kode_cabang)
        AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='TRANSIT OUT' OR tgl_mutasi_gudang_cabang>= (SELECT
            MAX(saldomax.tanggal)
            FROM saldoawal_bj saldomax
            WHERE saldomax.kode_cabang = mc.kode_cabang)
            AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='REJECT GUDANG' OR tgl_mutasi_gudang_cabang>=
                (SELECT MAX(saldomax.tanggal)
                FROM saldoawal_bj saldomax
                WHERE saldomax.kode_cabang = mc.kode_cabang)
                AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='REJECT PASAR' OR tgl_mutasi_gudang_cabang>=
                    (SELECT MAX(saldomax.tanggal)
                    FROM saldoawal_bj saldomax
                    WHERE saldomax.kode_cabang = mc.kode_cabang)
                    AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='REPACK' OR tgl_mutasi_gudang_cabang>=
                        (SELECT MAX(saldomax.tanggal)
                        FROM saldoawal_bj saldomax
                        WHERE saldomax.kode_cabang = mc.kode_cabang)
                        AND tgl_mutasi_gudang_cabang <= CURDATE() AND jenis_mutasi='PENYESUAIAN' GROUP BY kode_cabang )
                            mutasi"), function ($join) { $join->on('cabang.kode_cabang', '=', 'mutasi.kode_cabang');
                            }
                            );
                            ...
                            FROM saldoawal_bj saldomax
                            WHERE saldomax.kode_cabang = dpb.kode_cabang) AND tgl_pengambilan <= CURDATE() GROUP BY
                                kode_cabang ) dpb"), function ($join) { $join->on('cabang.kode_cabang', '=',
                                'dpb.kode_cabang');
                                }
                                );
                                ...
                                AND tgl_mutasi_gudang < CURDATE() GROUP BY kode_cabang ) mgudang"), function ($join) {
